...
import { Download, Volume2, Languages, Mic, Link, Play, AlertCircle, Send } from 'lucide-react';
import axios from 'axios';

// Telegram Send Button Component
function TelegramSendButton({ type, content, filename }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setError('Failed to send to Telegram');
    } finally {
      setSending(false);
    }
  };
...
// Telegram Send MP3 Button Component
function TelegramSendMP3Button({ audioData, caption }) {
  const [chatId, setChatId] = useState('');
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);
  const [error, setError] = useState('');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const sendResponse = await fetch('/api/telegram', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'sendAudio',
          chat_id: chatId,
          document: `data:audio/mpeg;base64,${base64}`,
          caption: caption,
        }),
      });
      ...
      setError('Failed to send MP3 to Telegram');
    } finally {
      setSending(false);
    }
  };
            ...
            <CardContent>
              ...
              <div className="flex flex-col gap-2">
                ...
                <TelegramSendButton 
                  ...
                  filename="english-transcription.txt"
                />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-primary/20">
            ...
            <CardContent>
              ...
              <div className="flex flex-col gap-2 mt-4">
                <Button size="sm" onClick={() => handleDownload('myanmar')}>
                  <Download className="mr-2 h-4 w-4" />
            ...
            <Card className="border-primary/20">
              ...
              <CardContent>
                ...
                <div className="flex flex-col gap-2 mt-4">
                  ...
                  <TelegramSendMP3Button 
                    audioData={results.audio}
                    caption="🎬 Myanmar Voiceover\nGenerated by Movie Recap Auto"
                  />
                </div>
              </CardContent>
            </Card>
